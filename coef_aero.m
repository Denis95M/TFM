function  [CX,CY,CZ,Cl,Cm,Cn] = coef_aero(x,u,geom,xcg)
   ...
   Cl = Clf(x(2),x(5)) + dldaf(x(2),x(5))*u(2)/20 + dldrf(x(2),x(5))*u(3)/30 + (D(5)*x(7)+D(6)*x(6))*geom(3)/(2*x(1));
   Cm = Cmf(x(2), u(1)) + geom(4)/(2*x(1))*x(3)*D(7) + CZ*(geom(5)-xcg);
   Cn = Cnf(x(2),x(5)) + dndaf(x(2),x(5))*u(2)/20 + dndrf(x(2),x(5))*u(3)/30 + (D(8)*x(7)+D(9)*x(6))*geom(3)/(2*x(1)) - CY*(geom(5)-xcg) * geom(4)/geom(3);
